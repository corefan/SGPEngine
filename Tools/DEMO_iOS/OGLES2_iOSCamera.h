...
class CIOSOpenGLES2Camera : public CMovementController
{
public:
   	CIOSOpenGLES2Camera(void)
	{
	...
	virtual ~CIOSOpenGLES2Camera(void) {}

    virtual void Update(float fElapsedTime)
	{
		Vector4D vcZoom, vcPan, VcNewLookAt;
		...
		Vector4D VcPos(GetPos());
		Vector4D VcDir(GetDir());
		Vector4D VcUp(GetUp());
		Vector4D VcRight(GetRight());
...
private:
    ...
    void RecalcAxes(void)
	{
		...
		if(m_fRotY > sgp::float_2pi) 
			m_fRotY -= sgp::float_2pi;
		else if(m_fRotY < -sgp::float_2pi) 
			m_fRotY += sgp::float_2pi;

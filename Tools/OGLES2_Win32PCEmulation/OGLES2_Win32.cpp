#include "OGLES2_Win32.h"
#include "OGLES2_Win32Camera.h"
...
public:
	...
	CWin32OpenGLES2Camera					m_GLES2Camera;
...
bool OGLESIntroducingPVRShell::InitApplication()
{
	...
	//}
	m_CreationParameters.WindowWidth = 1024;
	m_CreationParameters.WindowHeight = 768;
	m_CreationParameters.bRotated = false;
...
bool OGLESIntroducingPVRShell::InitView()
{
	...
	// Setting Working Dir
	String WorkingDir = File::getSpecialLocation(File::currentExecutableFile).getParentDirectory().getParentDirectory().getParentDirectory().getParentDirectory().getFullPathName();
	WorkingDir = WorkingDir + File::separatorString + String("Data2");
	...
	m_pRenderDevice->GetWorldSystemManager()->createNewWorld(m_pWorldMap, "newworld", SGPTS_SMALL, true, 80, String(L"Texture/terrain/dirt01_n.pvr") );

	// Create Water
	{
		// addWaterChunk must be called before creating Quadtree
		m_pRenderDevice->GetWorldSystemManager()->createWater(40.0f, String(L"Texture/water/wavesbump.pvr"));
...
*/
	...
	m_PlayerRole->setPosition(13.55f, m_pRenderDevice->GetWorldSystemManager()->getRealTerrainHeight(13.55f,3.55f) + 5.0f, 3.55f);
...
bool OGLESIntroducingPVRShell::RenderScene()
{
	...
	Vector2D *pMousePtr = (Vector2D *)PVRShellGet(prefPointerLocation);
	if( buttonState != 0 && pMousePtr != 0 )
	{
		if( !m_bMousePressed )
		{
			m_LastXPos = pMousePtr->x;
			m_LastYPos = pMousePtr->y;
			m_bMousePressed = true;			
		}
		if( buttonState == ePVRShellButtonMiddle )
		{
			if( m_pRenderDevice->isViewportRotated() )
			{
				m_GLES2Camera.SetPanY( (pMousePtr->x - m_LastXPos) * -13.0f );
				m_GLES2Camera.SetPanX( (pMousePtr->y - m_LastYPos) * 13.0f );
			}
			else
			{
				m_GLES2Camera.SetPanX( (pMousePtr->x - m_LastXPos) * -13.0f );
				m_GLES2Camera.SetPanY( (pMousePtr->y - m_LastYPos) * 13.0f );
			}
		}

		if( buttonState == ePVRShellButtonRight )
		{
			if( m_pRenderDevice->isViewportRotated() )
			{
				m_GLES2Camera.SetRotationSpeedY((pMousePtr->y - m_LastYPos) * -30.0f);
				m_GLES2Camera.SetRotationSpeedX((pMousePtr->x - m_LastXPos) * 30.0f);
			}
			else
			{
				m_GLES2Camera.SetRotationSpeedX((pMousePtr->y - m_LastYPos) * -30.0f);
				m_GLES2Camera.SetRotationSpeedY((pMousePtr->x - m_LastXPos) * 30.0f);
			}
		}
		m_LastXPos = pMousePtr->x;
		m_LastYPos = pMousePtr->y;
	...
	m_pRenderDevice->DrawTextInPos( 10, 10, SGPFDL_SHADOW, 16, 200, 200, 200, L"²âÊÔFPS =  %f ", 1000.0 / m_AverageMS );
		
		//m_pRenderDevice->DrawTextInPos( 10, 30, SGPFDL_DEFAULT, 16, 200, 200, 200, L"Very detailed Chunk Num = %d ",
		//		static_cast<COpenGLES2RenderDevice*>(m_pRenderDevice)->getOpenGLTerrainRenderer()->getVeryDetailedChunkNumber() );
		//m_pRenderDevice->DrawTextInPos( 10, 50, SGPFDL_DEFAULT, 16, 200, 200, 200, L"LOD0 Chunk Num = %d ",
		//		static_cast<COpenGLES2RenderDevice*>(m_pRenderDevice)->getOpenGLTerrainRenderer()->getLOD0ChunkNumber() );
		//m_pRenderDevice->DrawTextInPos( 10, 70, SGPFDL_DEFAULT, 16, 200, 200, 200, L"LOD1 Chunk Num = %d ",
		//		static_cast<COpenGLES2RenderDevice*>(m_pRenderDevice)->getOpenGLTerrainRenderer()->getLOD1ChunkNumber() );
		//m_pRenderDevice->DrawTextInPos( 10, 90, SGPFDL_DEFAULT, 16, 200, 200, 200, L"Blend LOD Chunk Num = %d ",
		//		static_cast<COpenGLES2RenderDevice*>(m_pRenderDevice)->getOpenGLTerrainRenderer()->getLODBlendChunkNumber() );
